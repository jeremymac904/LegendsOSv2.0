...
export default async function AdminCenterPage() {
  ...
  // ------------------------------------------------------------------
  // Tabbed body. The long card stack is collapsed into four switchable
  // panels so the landing reads as a compact console instead of an endless
  // scroll. SectionHeader + connection board + stat row stay pinned above as
  // the always-visible glance; everything heavier lives behind the tabs.
  const tabs: TabItem[] = [
    {
      id: "usage",
      label: "Live usage",
            ...
            <ul className="mt-4 space-y-2 text-sm">
              {liveStatuses.map((p) => {
                ...
                return (
                  <li
                    key={p.id}
                    className="flex items-center justify-between rounded-lg border border-ink-200 bg-white/70 px-3 py-2 dark:border-ink-800 dark:bg-ink-900/50"
                  >
                    <div>
            ...
            <div className="mt-3 grid gap-2">
              {recentChats.length === 0 ? (
                ...
                recentChats.map((t) => (
                  <div
                    key={t.id}
                    className="flex items-center justify-between rounded-lg border border-ink-200 bg-white/70 px-3 py-2 text-xs dark:border-ink-800 dark:bg-ink-900/40"
                  >
            ...
            <div className="mt-3 grid gap-2">
              {recentSocial.length === 0 ? (
                ...
                recentSocial.map((s) => (
                  <div
                    key={s.id}
                    className="rounded-lg border border-ink-200 bg-white/70 px-3 py-2 text-xs dark:border-ink-800 dark:bg-ink-900/40"
                  >
          ...
          {ownerConnectionCards.map((card) => {
            ...
            return (
              <Link
                key={card.title}
                href={card.href}
                className="rounded-xl border border-ink-200 bg-white/70 p-3 transition hover:border-accent-gold/30 dark:border-ink-800 dark:bg-ink-900/40"
              >
                <div className="flex items-center justify-between gap-2">
    ...
    <section className="card-padded">
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 lg:grid-cols-3">
        {tiles.map((t) => (
          <div
            key={t.label}
            className="rounded-xl border border-ink-200 bg-white/70 p-3 dark:border-ink-800 dark:bg-ink-900/40"
          >
