        ...
        <li key={f.id}>
          <button
            ...
            className={cn(
              ...
              active === f.id
                ? "border-accent-champagne/30 bg-ink-800/40 text-ink-100"
                : "border-ink-800 bg-ink-900/40 text-ink-200 hover:border-accent-champagne/20"
            )}
          >
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[280px_1fr]">
        <div className="space-y-3">
          <div className="card-padded">
            ...
            <div className="mt-1.5 flex items-center gap-2 rounded-xl border border-ink-700/80 bg-ink-950/50 px-2.5">
              <Search size={14} className="text-ink-400" />
              <input
                value={search}
                onChange={(e) => runSearch(e.target.value)}
                placeholder="Name or loan #"
                className="w-full bg-transparent py-2 text-sm text-ink-100 outline-none placeholder:text-ink-500"
              />
            </div>
          </div>
                ...
                {roots.map((r) => (
                  <li key={r.id}>
                    <button
                      ...
                      onClick={() => openRoot(r)}
                      className={cn(
                        "group flex w-full items-center justify-between rounded-xl border border-transparent px-3 py-2 text-sm transition-colors hover:bg-ink-800/30",
                        selectedRoot?.id === r.id
                          ? "border-accent-champagne/25 bg-ink-800/30 text-ink-100"
                          : "text-ink-300"
                      )}
                    >
                      <span className="flex items-center gap-2">
        ...
        <div className="space-y-4">
          {!selectedRoot && !selectedFolder && (
            <div className="card flex flex-col items-center justify-center gap-3 p-10 text-center">
              <span className="grid h-12 w-12 place-items-center rounded-full border border-accent-gold/30 bg-ink-900/70 text-accent-gold">
                <FolderTree size={20} />
              </span>
              <div>
          ...
          {selectedFolder && (
            <>
              <div className="flex items-center gap-2 text-xs text-ink-400">
                ...
                <span className="text-ink-200">{selectedFolder.label}</span>
              </div>
              ...
              <div className="card-padded">
                ...
                {previewFile && (
                  <div className="mt-3 rounded-xl border border-ink-800 bg-ink-950/60 p-4">
                    ...
                    <div className="mt-2 grid h-40 place-items-center rounded-lg border border-dashed border-ink-700 bg-ink-900/40 text-center">
                      <div>
                        <FileText size={22} className="mx-auto text-ink-500" />
